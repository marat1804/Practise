...
TEST(Add_contact, All) {
	...
	ASSERT_NO_THROW(circuit.add_contact(s));
	ASSERT_NO_THROW(circuit.add_contact(1, 0, 5));
}

TEST(Costructor, Copy) {
	Prog::Circuit_Board circuit;
	ASSERT_NO_THROW(circuit.add_contact(1, 2, 3));
	ASSERT_NO_THROW(circuit.add_contact(1, 5, 6));
	ASSERT_NO_THROW(circuit.add_contact(0, 8, 7));
...
TEST(Constructor, Moving) {
	...
	Prog::Circuit_Board  c1 = std::move(circuit);

	ASSERT_EQ(c1.check_connetcion(0, 1), false);
	ASSERT_THROW(circuit.check_connetcion(0, 1), std::exception);
}
...
TEST(Minus_Sign, Operator_Overload) {
	...
	ASSERT_THROW(c1 - 4, std::exception);

	c1 += s1;
	c1 += s2;
	c1 += s3;
	c1.add_connection(0, 1);
	ASSERT_THROW(c1 - 6, std::exception);
	ASSERT_NO_THROW(c1 - 0);

	ASSERT_NO_THROW(c1.add_connection(0, 1));

	ASSERT_NO_THROW(c1 - 1);
}
...
TEST(Assignment, Copy) {
	Prog::Circuit_Board c1, c2;
	Prog::contact s1(1, 1, 1), s2(0, 0, 0), s3(1, 2, 3);
	c1 += s1;
	c1 += s2;
	ASSERT_EQ(c1.check_connetcion(0, 1), true);
	c2 += s1;
	c2 += s3;
	ASSERT_EQ(c2.check_connetcion(0, 1), false);
	c1 = c2;
...
TEST(Assignment, Move) {
	...
	c1 = std::move(c2);
	ASSERT_EQ(c1.check_connetcion(0, 1), false);
	ASSERT_EQ(c2.check_connetcion(0, 1), true);
}
