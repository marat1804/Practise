...
const char *menu[] = {"0. Exit", "1. Add contact on circuit", "2. Add connection", "3. Route distance", "4. Print", "5. Print chosen type" };
	...
	while (fl) {
		...
		case 1:
			...
			std::cout << "To add a new contact please enter type (0 - out, 1 - in) and coordinates (x,y) --> ";
				//std::cin >> elem.type >> elem.x >> elem.y;
				/*if (!std::cin.good()) {
					std::cin.clear();
					std::cin.ignore(10000, '\n');
		...
		case 3:
			...
			std::cin >> from >> to;
			try {
				dist = circuit.route_length(from, to);
